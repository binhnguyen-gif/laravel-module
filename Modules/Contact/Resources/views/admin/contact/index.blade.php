...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
{{--                    <h4 class="header-title mt-0 mb-1">Contact</h4>--}}
                    <p class="sub-header">
                    </p>
                    ...
                    <table  class="table m-0">
                        ...
                        @foreach($items as $item)
                            <tr>
                                ...
                                <td>{{ $item->email }}</td>
                                <td>{{ $item->phone }}</td>
                                <td style="max-width: 150px">{{ $item->subject }}</td>
                                <td style="max-width: 350px">{{ $item->message }}</td>
                                <td>{{ $item->created_at }}</td>
                                <td class="text-right">
{{--                                    <a class="btn btn-soft-primary btn-sm" href="{{ route('blog.admin.post.edit', $item->id) }}">--}}
{{--                                        <i class="fa fa-edit"></i>--}}
{{--                                    </a>--}}
                                    <x-form-delete :item="$item" url="{{ route('contact.admin.destroy', $item->id) }}"></x-form-delete>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
